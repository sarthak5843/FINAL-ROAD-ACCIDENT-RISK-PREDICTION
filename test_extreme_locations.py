...
for city in cities:
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{city['name']"12"}: {temp"6.1f"}°C, {weather_main}, Lat: {city['lat']"6.2f"}")
        else:
            print(f"{city['name']"12"}: API Error {response.status_code}")
    except Exception as e:
        print(f"{city['name']"12"}: Error - {str(e)[:30]}...")
...
for city in cities:
    ...
    print(f"{city['name']"12"}: Risk {risk".1f"} ({geo}, {weather}, {time_factor})")
